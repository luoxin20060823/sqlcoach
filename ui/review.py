...
def render_review_tab(llm_client, store):
    ...
    if not wrong:
        st.info("还没有错题记录。继续做题吧～")
        return

    st.caption(f"共 {len(wrong)} 道错题。点列表中任意一题，重新作答。")
    ...
    pick = st.selectbox("选择一道错题重新作答", options)
    if not pick:
        return
    ...
    if st.session_state.get(res_key):
        ...
        icons = {"correct": "✅ 这次对了！", "wrong": "❌ 仍然错误", "flawed": "⚠️ 结果对但逻辑有瑕疵"}
